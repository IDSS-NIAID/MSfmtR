...
# Write data to excel
file_out <- "output/20240117_101231_aspinwallja-20230825-PI1_3reps_Report.xlsx"

wb <- createWorkbook()
addWorksheet(wb, 'aspinwallja-20230825-PI1')
...
for(i in 1:3)#dim(proteins)[1])
{
  proteins[i,-which('peptides' == names(proteins))] %>%
    writeData(wb = wb, sheet = 'aspinwallja-20230825-PI1', startRow = nextRow, startCol = 1, rowNames = FALSE,
              colNames = i == 1)
  ...
  proteins$peptides[[i]] %>%
    select(-PROTEIN) %>%
  writeData(wb = wb, sheet = 'aspinwallja-20230825-PI1', startRow = nextRow + 1, startCol = 2, rowNames = FALSE)
  groupRows(wb, 'aspinwallja-20230825-PI1', nextRow + 0:dim(proteins$peptides[[i]])[1], hidden = TRUE)
