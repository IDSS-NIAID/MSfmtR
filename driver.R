...
foreach(j=1:length(paths)) %dopar%
{
  ...
  if(file.exists(paste0('output/', dirs[j], '_wb.RData')))
  {
    load(paste0('output/', dirs[j], '_wb.RData'))
  }else{
    wb <- createWorkbook()
    ...
    # Write data to excel
    #openXL(wb)
    save(wb, indices, file = paste0('output/', dirs[j], '_wb.RData'))
  }
  ...
  # format proteins
  addStyle(wb = wb, sheet = dirs[j], style = protein_header_style,
           rows = 1, cols = 1:dim(proteins)[2],
           gridExpand = TRUE)
  addStyle(wb = wb, sheet = dirs[j], style = protein_rows_style,
           rows = indices$protein_rows,
           cols = 1:dim(proteins)[2],
           gridExpand = TRUE)
  
  # format peptides
  addStyle(wb = wb, sheet = dirs[j], style = peptide_header_style,
           rows = indices$peptide_headers,
           cols = 2:max(which(!is.na(proteins[2,]))),
           gridExpand = TRUE)
  addStyle(wb = wb, sheet = dirs[j], style = peptide_rows_style,
  ...
  # save this monster
  saveWorkbook(wb, file = paste0('output/', dirs[j], '.xlsx'))
